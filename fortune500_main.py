...
# Define id_fortune500_2021 (you need to define it before using it)
id_fortune500_2021 = 'your_google_drive_file_id_for_2021'  # replace with the actual file ID

# Download the file for 2021 from Google Drive
...
# Create dataframe for the 2021 file
f500_2021 = pd.read_csv(f500_2021_localname_csv)
...
# Clean NaN values in the 'Profit (in millions)' column
f500['Profit (in millions)'] = f500['Profit (in millions)'].replace("N.A.", np.NaN)
f500['Profit (in millions)'].fillna(0, inplace=True)
...
# Convert 'Revenue' column to float in f500_all
f500_all = f500_all.astype({"Revenue": float})
print(f500_all.dtypes)

# Seaborn plot with adjusted parameters
plt.figure(figsize=(12, 6))
sns.scatterplot(data=f500, x='Year', y='Revenue', hue='Industry', size='Profit (in millions)', alpha=0.7, palette='viridis')
...
# Constant (intercept) to the model
X = sm.add_constant(X)
